...
export default function LiqudityLockTest(props) {
  ...
  useEffect(() => {
    if (selfClick) {
      ...
      props.setParentSymbol(symbol);
      props.setParentUnlockTime(unlocktime);
      props.setParentModalOpen(opendis);
      setSelfClick(false);
    }
  }, [name, symbol, unlocktime, opendis]);
  ...
  const getPrice = async () => {
    ...
    try {
      ...
      props.setParentPrice(ethers.utils.formatUnits ( tprice, 18 ));
      props.setParentPenalFee(tpenaltyfee.toString());
    } catch (error) {
      return;
    }
  }
        ...
        <Card>
          ...
          <CardBody>
            ...
            <Box sx={{ display: 'flex', alignItems: 'flex-end' }}>
              ...
              <TextField fullWidth id="input-with-sx" value = {tokenstaddr} onFocus={focusInputHandle} onChange={(e) => addrChange(e.target.value)} label="Input token address" variant="standard" />
            </Box>
